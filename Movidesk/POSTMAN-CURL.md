...
## 1️⃣ e 2️⃣ SINCRONIZAR TICKETS — não existe mais como endpoint HTTP
A sincronização com a API do Movidesk saiu do servidor web. Agora ela roda como
script separado (fora do processo do Express), agendado no Windows Task Scheduler:
```bash
node scripts/sync-movidesk.js incremental
node scripts/sync-movidesk.js full
```
O servidor e todas as rotas HTTP (inclusive `/api/tickets`) só leem a tabela
`tickets` do Postgres.
