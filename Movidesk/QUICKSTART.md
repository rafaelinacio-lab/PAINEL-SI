...
| GET | /api/tickets | Listar todos os tickets (lê só o banco) |
| GET | /api/tickets/:id | Detalhes de um ticket |
| GET | /api/tickets/stats/overview | Estatísticas |

Sincronizar com o Movidesk não é mais um endpoint HTTP — roda via `node scripts/sync-movidesk.js` (fora do servidor, agendado no Windows Task Scheduler).
