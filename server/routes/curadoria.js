...
// ===== Sincronização de Módulo x Rotina (campo customizado do Movidesk) =====
// O campo customizado 59786 do Movidesk (ex: "Folha > Férias") não vem no payload padrão
// de sincronização de tickets — só buscando o ticket individualmente com $select=customFieldValues.
// Mesma arquitetura da sincronização de satisfação: busca chamado por chamado, fila de
// pendentes, respeitando o rate limit (throttle compartilhado do Movidesk).
...
// Busca os campos customizados de UM chamado específico, com retry cobrindo tanto
// respostas HTTP de erro quanto falhas de rede. `selectFields` e `rateLimitMs` vêm de
// curadoria_movidesk_config (Configurações → Curadoria Avançado).
async function fetchTicketCustomFields(token, ticketId, movideskCfg, attempt = 0) {
  const rateLimitMs = movideskCfg.rateLimitMs || MOVIDESK_MIN_INTERVAL_MS;
  await throttleMovideskRequest(rateLimitMs);
  const selectFields = movideskCfg.moduloRotina?.selectFields || 'id,customFieldValues';
  const url = `${API_TICKET_URL}?token=${encodeURIComponent(token)}&id=${ticketId}&$select=${selectFields}`;
  ...
  try {
    ...
    if (attempt < 4) {
      await new Promise(r => setTimeout(r, (attempt + 1) * rateLimitMs));
      return fetchTicketCustomFields(token, ticketId, movideskCfg, attempt + 1);
    }
    throw new Error(`Falha de rede ao buscar campos customizados do chamado ${ticketId}: ${networkErr.message}`);
  }
  ...
  if (!res.ok) {
    ...
    throw new Error(`Falha ao buscar campos customizados do chamado ${ticketId}: ${res.status}`);
  }
  const data = await res.json();
  return data?.customFieldValues || [];
}

function extractModuloXRotina(customFieldValues, customFieldId = MODULO_CUSTOM_FIELD_ID) {
  const field = (customFieldValues || []).find(f => f.customFieldId === customFieldId);
  const item = field?.items?.[0]?.customFieldItem;
...
async function runModuloSyncLoop() {
  try {
    ...
    for (const row of pending.rows) {
      ...
      try {
        const customFieldValues = await fetchTicketCustomFields(token, row.ticket_id, movideskCfg);
        const modulo = extractModuloXRotina(customFieldValues, movideskCfg.moduloRotina?.customFieldId);
        const verificadoEm = new Date().toISOString();
